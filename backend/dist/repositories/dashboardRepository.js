...
const getAppointmentSummaryRepository = async (doctorId, startDate) => {
...
const getPaymentSummaryRepository = async (doctorId, startDate) => {
    const appointments = await appointment_1.default.find({
        doctorId,
        date: { $gte: startDate.toISOString() },
    });
...
const getPopularTimeSlotsRepository = async (doctorId, startDate) => {
    const timeSlots = await appointment_1.default.aggregate([
        {
            $match: {
                doctorId,
                date: { $gte: startDate.toISOString() },
...
const getMonthlyStatsRepository = async (doctorId, startDate) => {
...
const getTodayAppointmentsRepository = async (doctorId) => {
